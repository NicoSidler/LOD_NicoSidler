...
WHERE {
    ### subquery adding the distinct clause
    {
        ...
        WHERE {
            ...
            # Updated date range: 1801 to 1990
            FILTER(xsd:integer(?year) >= 1801 && xsd:integer(?year) <= 1990)

            # Updated population: Sociologist or Sociology
...
WHERE {
    ...
    # Filters out any organisation that HAS an English label
    MINUS {
        ?organisation_uri rdfs:label ?organisation_lab.
        FILTER(LANG(?organisation_lab) = 'en')
    }
    
    # Gets the label in other available languages
